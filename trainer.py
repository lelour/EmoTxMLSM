...
class trainer(object):
    def __init__(self, config):
        ...
        elif config["model_no"] == 4.0 or config["model_no"] == 5.0:
        ...
        model_id2st_map = {
            ...
            "3.0": "Emotx_1CLS",
            "4.0": "Emotx",
            "5.0": "EmoTx_MultiLabelSoftMarginLoss"
        }
    ...
    def config_sanity_check(self):
        ...
        assert((self.config["model_no"] in [3.0, 4.0, 5.0] and self.config["use_srt_feats"]) or not self.config["use_srt_feats"])
        assert(self.config["model_no"] not in [3.0, 4.0, 5.0] or (self.config["model_no"] in [3.0, 4.0, 5.0] and \
    ...
    def trigger_training(self):
        ...
        if config["model_no"] == 5.0:
            criterion = nn.MultiLabelSoftMarginLoss().to(self.device)
        else:
...
def fill_model_name(config):
    ...
    if float(config["model_no"]) in [3.0, 4.0, 5.0]:
